 ...
 * String query = "SELECT * FROM progra2.users u WHERE u.email='" + text +
 * "' OR u.phone_number = '" + text + "'";
 ...
 * 
 * return stal;
 * //hago un objeto en user, que recibe el texto del gui, y ademas la concexion
 * de DBconnecntion
 * //ademas creo un query que la uso para realizar la busqueda
 * }
 * public boolean buscar(String text, Dbconnection conn) {
 * String query = "SELECT COUNT(id) FROM progra2.users u WHERE u.email='" + text
 * + "' OR u.phone_number = '" + text + "'";
 * ResultSet rset = conn.select(query);
 * System.out.println(query);
 * int n;
